      ...
      try {
        ...
        if (u) {
          ...
          if (!websocket) {
            ...
            const socketUrl = socketProtocol + "//creativeu-app.live/connect"; //+ "localhost:4242/connect"; for development
      ...
      registerRequest(userObj)
        .then((u) => {
          ...
          if (!websocket) {
            const socketProtocol =
              window.location.protocol === "https:" ? "wss:" : "ws:";
            const socketUrl = socketProtocol + "//creativeu-app.live/connect"; //+ "localhost:4242/connect";
            const socket = new WebSocket(socketUrl);
